...
print(f"{len(points)} points with coordinates")

coord_str = ";".join(f"{p['lon']:.6f},{p['lat']:.6f}" for p in points)
...
out = {
    ...
    "distances": result["distances"],  # meters, [i][j]
...
print(f"wrote {OUT_JSON}: {len(points)}x{len(points)} driving distance/duration matrix")
